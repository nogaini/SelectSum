...
def add_topic_to_segments(
...
) -> tuple[list[dict], dict[int, list[str]], list[str]]:
    ...
    for topic_id, segment in zip(topics, segments):
        ...
        topic_tags_list.add(segment_with_topic["topic_tags"])

    topic_tags_list = list(topic_tags_list)
    return segments_with_topics, topic_dict, topic_tags_list
